...
var findChoice = function findChoice(replaceChoices, tag) {
  return _.find(replaceChoices, function (c) {
    return c.value === tag;
  });
};
...
var PrettyTextInputHelper = createReactClass({
  ...
  getInitialState: function getInitialState() {
    ...
    var blocks = draftJs.convertFromRaw(rawContent);
    var decorator = new draftJs.CompositeDecorator([{
      strategy: findTagEntities,
      component: TagComponent
    }]);

    var editorState = draftJs.EditorState.createWithContent(blocks, decorator);
  ...
  handleChoiceSelection: function handleChoiceSelection(tag) {
    ...
    var selectionState = entityRange ? draftJs.SelectionState.createEmpty(blockKey).merge(entityRange) : editorState.getSelection();

    var choice = findChoice(replaceChoices, tag);
    var label = getChoiceLabel(choice, tag, humanize);

    var contentStateWithReplacedText = draftJs.Modifier.replaceText(contentState, selectionState, label);
    ...
    var currentSelectionState = entityRange ? draftJs.SelectionState.createEmpty(blockKey) : editorState.getSelection();
    ...
    var newContentState = draftJs.Modifier.applyEntity(contentStateWithEntity, newSelectionState, newEntityKey);

    var newEditorState = draftJs.EditorState.push(editorState, newContentState);
  ...
  renderDefault: function renderDefault() {
    ...
    var textBoxClasses = cx(_.extend({}, classes, {
