  ...
  return {
    ...
    createChildFields_Array: function (field) {
      ...
      return field.value.map(function (arrayItem, i) {
        const childFieldTemplate = config.childFieldTemplateForValue(field, arrayItem);
    ...
    createChildFields_AssocList: function (field) {
      ...
      return field.value.map(function (row, i) {
        const childFieldTemplate = config.childFieldTemplateForValue(field, row.value);
    ...
    createChildFields_Object: function (field) {
      ...
      return Object.keys(field.value).map(function (key, i) {
        const childFieldTemplate = config.childFieldTemplateForValue(field, field.value[key]);

        const childField = config.createChildField(field, {
          fieldTemplate: childFieldTemplate, key, fieldIndex: i, value: field.value[key]
        });
    ...
    renderFormaticComponent: function (component) {
      ...
      return (
        <div className="formatic">
          {config.createFieldElement({field, onChange: component.onChange, onAction: component.onAction})}
        </div>
      );
    },
    ...
    createRootField: function (props) {
      ...
      let fieldTemplate = props.fieldTemplate || props.fieldTemplates || props.field || props.fields;
      ...
      const field = _.extend({}, fieldTemplate, {rawFieldTemplate: fieldTemplate});
    ...
    createNewChildFieldValue: function (parentField, itemFieldIndex) {
      ...
      const childFieldTemplate = config.fieldItemFieldTemplates(parentField)[itemFieldIndex];
      ...
      const childField = config.createChildField(parentField, {
        fieldTemplate: childFieldTemplate, key, fieldIndex, value: newValue
      });
