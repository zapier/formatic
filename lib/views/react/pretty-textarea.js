...
module.exports = React.createClass({
  ...
  indexMap: function (tokens) {
    ...
    _.each(tokens, function (token, tokenIndex) {
      if (token.type === 'tag') {
        var label = LEFT_PLAIN_PAD + noBreak(this.prettyLabel(token.value)) + RIGHT_PLAIN_PAD;
  ...
  plainValue: function (value) {
    ...
    return parts.map(function (part) {
      if (part.type === 'text') {
        ...
        return LEFT_PLAIN_PAD + noBreak(this.prettyLabel(part.value)) + RIGHT_PLAIN_PAD;
      }
    }.bind(this)).join('');
  },
  //
