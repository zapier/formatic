...
module.exports = React.createClass({
  ...
  shouldComponentUpdate: function(nextProps, nextState) {
    // TODO: include replaceChoices, whatever else that we need that might change
    return (nextState.value !== this.state.value) || (nextState.codeMirrorMode && !this.state.codeMirrorMode);
  },
  ...
  componentWillReceiveProps: function(nextProps) {
    ...
    // TODO: this is really wonky but we're going to get rid of initialValue shortly anyway.
    // Revisit once initialValue is gone.
    else if (!this.state.value && nextProps.field.initialValue && this.state.value !== nextProps.field.initialValue) {
      this.setState({ value: nextProps.field.initialValue });
    }
  },
  ...
  renderDefault: function () {
    ...
    var element = (
      <div onMouseEnter={this.switchToCodeMirror}>
        ...
        <ChoicesDropdown ref="choicesDropdown" choices={choices} handleSelection={this.handleChoiceSelection} />
      </div>
    );
  ...
  displayChoicesDropdown: function () {
