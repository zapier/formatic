...
module.exports = React.createClass({
  ...
  renderDefault: function () {
    ...
    var element = (
      <div className='pretty-text-wrapper' onMouseEnter={this.switchToCodeMirror}>
  ...
  getTagClasses: function (tag) {
    var choice = _.find(this.state.replaceChoices, (c) => c.value === tag);
  ...
  createReadonlyEditor: function () {
    ...
    var nodes = tokens.map(function (part) {
      if (part.type === 'tag') {
        ...
        return <PrettyTag classes={tagClasses}>{self.state.translator.getLabel(part.value)}</PrettyTag>;
      }
  ...
  createTagNode: function (pos) {
    ...
    React.render(
      <PrettyTag classes={tagClasses} onClick={onTagClick.bind(this)}>{label}</PrettyTag>,
      node
    );
    return node;
  }
