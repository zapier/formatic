  ...
  renderDefault: function () {
    ...
    if (!this.isReadOnly() && fieldTemplates.length > 0) {
      typeChoices = config.createElement('field-template-choices', {
        field, fieldTemplateIndex: this.state.fieldTemplateIndex, onSelect: this.onSelect
      });
    }

    let addItem;
    if (!this.isReadOnly()) {
      addItem = config.createElement('add-item', {field, onClick: this.onAppend, tabIndex: this.props.tabIndex});
    }
