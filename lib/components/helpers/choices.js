...
module.exports = React.createClass({
  ...
  componentWillReceiveProps: function (nextProps) {
    ...
    this.setState(nextState, () => {
      if (isOpening && this.refs.search) {
        this.refs.search.focus();
      }
      if (isClosing && this.props.onFocusSelect) {
  ...
  hasSearch(visibleChoices = this.visibleChoices()) {
  ...
  renderDefault: function () {
    ...
    if (!hasSearch) {
      if (this.props.choices.length > 2) {
        if (_.find(choices, choice => !choice.action && choice.value !== '///loading///')) {
          hasSearch = true;
        }
      }
    }
        ...
        R.ul({key: 'choices', ref: 'choices', className: 'choices'},
          ...
          choices.map(function (choice, i) {
            ...
            return config.createElement('choices-item', {
              key: i, isHovering: this.state.hoverValue && this.state.hoverValue === choiceValue
            }, choiceElement);
