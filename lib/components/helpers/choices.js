  ...
  visibleChoices: function () {
    ...
    if (this.state.searchString) {
      choices = choices.filter((choice) => {
        ...
        return choice.label && choice.label.toLowerCase().indexOf(this.state.searchString.toLowerCase()) > -1;
      });
    }
    ...
    if (this.props.open) {
      return R.div({ref: 'container', onWheel: this.onWheel, onScroll: this.onScroll, onClick: this.onClick,
        ...
        config.cssTransitionWrapper(
          R.div({key: 'search', className: 'choices-search'},
            R.input({type: 'text', placeholder: 'Search...', onChange: this.onChangeSearch})
          ),
