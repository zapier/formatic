...
export default createReactClass({
  ...
  getInitialState: function() {
    ...
    const translator = TagTranslator(selectedChoices.concat(replaceChoices), this.props.config.humanize);
  ...
  renderDefault: function () {
    const textBoxClasses = cx(_.extend({}, this.props.classes, {
    ...
    return (
      <div role="presentation" onKeyDown={this.onKeyDown} className={cx({'pretty-text-wrapper': true, 'choices-open': this.state.isChoicesOpen})}
        onMouseEnter={this.switchToCodeMirror} onTouchStart={this.switchToCodeMirror}>
        <div className="pretty-text-click-wrapper"
          tabIndex="0"
          ...
          role="textbox">
          <div role="presentation" className={textBoxClasses} tabIndex={this.wrapperTabIndex()} onBlur={this.onBlur}>
  ...
  tagCodeMirror: function (cursorPosition) {
    const positions = this.state.translator.getTagPositions(this.codeMirror.getValue());
    const self = this;
  ...
  createReadonlyEditor: function () {
    ...
    const nodes = tokens.map(function (part, i) {
      if (part.type === 'tag') {
        ...
        const props = {key: i, tag: part.value, replaceChoices: self.state.replaceChoices, field: self.props.field};
  ...
  createTagNode: function (pos) {
    ...
    const props = {onClick: this.onTagClick, field: this.props.field, tag: pos.tag};
