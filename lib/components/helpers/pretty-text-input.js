...
export default React.createClass({
  ...
  getInitialState: function() {
    console.log('pretty-text-input getInitialState');
  ...
  handleChoiceSelection: function (key, event) {
    const selectChoice = () => {
      ...
      console.log('handleChoiceSelection 1');
    ...
    if (this.state.codeMirrorMode) {
      ...
      console.log('hackety hack to stop propogation wow!');
      event.stopPropagation();
      this.isInserting = true;
      this.onChange('{{' + key + '}}');
      this.isInserting = false;
      console.log('handleChoiceSelection 2');
  ...
  onFocusClick: function () {
    console.log('--- xxxx onFocusClick, isChoicesOpen', this.state.isChoicesOpen);
    if (!this.state.isChoicesOpen) {
      event.stopPropagation();
      this.setState({ hasFocus: true, selectedTagPos: null, isChoicesOpen: true });
      this.onStartAction('open-replacements');
    } else {
      console.log('onFocusClick, isChoicesOpen false');
    }
  ...
  focusCodeMirror() {
    console.log('focusCodeMirror');
    if (this.codeMirror) {
      this.codeMirror.focus();
    }
  },

  onFocusCodeMirror(event) {
    console.log('---- hey onFocusCodeMirror, event:', event);
    this.props.onFocus();
  },
  ...
  choices: function () {
    ...
    return this.props.config.createElement('choices', {
      ref: 'choices',
      clickOutsideRef: this.refs.prettyTextInputContainer,
  ...
  renderDefault: function () {
    ...
    return (
      <div ref="prettyTextInputContainer" onKeyDown={this.onKeyDown} className={cx({'pretty-text-wrapper': true, 'choices-open': this.state.isChoicesOpen})}
  ...
  getCloseIgnoreNodes: function () {
    return [this.refs.prettyTextInputContainer];
  },

  onToggleChoices: function () {
    console.log('onToggleChoices');
    this.setChoicesOpen(!this.state.isChoicesOpen);
  },

  setChoicesOpen: function (isOpen) {
    console.log('setchoicesopen', isOpen);
  ...
  onCloseChoices: function () {
    console.log('onCloseChoices, isChoicesOpen', this.state.isChoicesOpen);
    if (this.state.isChoicesOpen) {
      console.log('onCloseChoices, closing it');
