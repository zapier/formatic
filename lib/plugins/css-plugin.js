...
const plugin = prevConfig => {
  ...
  const renderTags = {
    Array_AddItem: (tagName, tagProps, metaProps, ...children) => {
      return origRenderTag('button', tagProps, metaProps, ...children);
    },
    Array_RemoveItem: (tagName, tagProps, metaProps, ...children) => {
      return origRenderTag('button', tagProps, metaProps, ...children);
    },
  };
  ...
  return {
    renderTag: (tagName, tagProps, metaProps, ...children) => {
      ...
      if (renderTags[fieldElementName]) {
        return renderTags[fieldElementName](
          tagName,
          tagProps,
          metaProps,
          ...children
